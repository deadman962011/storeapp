    ...
    public function getAttachmentAttribute(){
        ...
        if($this->item_type==='list'){
            ...
            if($this->attachment_type==='category'){
                ...
                $attachment['products']=storeProduct::active()->parent()->where('product_category',$attachment->id)->limit($this->item_items_count)->without(['metas'])->get();
            }
            elseif($this->attachment_type==='brand'){
                $attachment=productBrand::find($this->attachment_id);
                $attachment['products']=storeProduct::active()->parent()->where('product_brand',$attachment->id)->limit($this->item_items_count)->without(['metas'])->get();
            }
