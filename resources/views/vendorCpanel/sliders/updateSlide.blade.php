            ...
            <div class="content-wrapper">
                ...
                <div class="content">
                    <div class="container-fluid">
                        <form action="{{ route('slide.edit.post', ['id' => $slider->id,'slideId'=>old('itemId'),'lang'=>old('language')]) }}" method="POST">
...
@section('script')
    ...
    <script>
        $(document).ready(function() {
            ...
            var action = $('#slide_action_type').val();
            if (action === 'link') {
                linkInput.show()
                selectInput.hide()
            } else {

                console.log('am here',selectInput)
                selectInput.show()
                linkInput.hide()
                    ...
                    success: function(resp) {
                        if (resp.success) {
                            ...
                            if (items.length > 0) {
                                items.forEach(item => {
                                    var option =
                                        `<option value='${item.id}'>${item.strings[stringName]}</option`;
